...
def interection(results):
    ...
    for i in range(len(results)):
        img = mpimg.imread(
            "runs/detect/predict/image" + str(i + 1) + ".jpg"
        ...
        for box in result.boxes:
            ...
            class_name = result.names[
                class_id
            ]  # Obtenha o nome da classe a partir de seu ID
            boxes.append(Retangulo(xmin, ymin, xmax, ymax, conf, class_name))
        for i in range(len(boxes)):
            for j in range(i + 1, len(boxes)):
                if (
                    overlap(boxes[i], boxes[j])
                    and boxes[i].class_name != boxes[j].class_name
                ):
                    if (
                        boxes[i].class_name == "Man" and boxes[j].class_name == "Woman"
                    ) or (
                        boxes[i].class_name == "Woman" and boxes[j].class_name == "Man"
                    ):
                        pass
                    else:
                        print(
                            f"Interaction between : {boxes[i].class_name} and {boxes[j].class_name}"
                        )
